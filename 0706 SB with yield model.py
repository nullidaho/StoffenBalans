...
<<<<<<< HEAD
        ...
        soil_keys = []
        for s, r in zip(soil_series, region_series):
            # 🌟 CORE FIX: str() conversion prevents TypeError when facing NaN floats from empty Excel rows
            s_str = str(s).lower().strip()
            r_str = str(r).lower().strip()
            
            if 'klei' in s_str: 
                soil_keys.append('klei')
            elif 'veen' in s_str or 'peat' in s_str: 
                soil_keys.append('veen')
            elif any(term in s_str for term in ('loss', 'loes', 'löss')): 
                soil_keys.append('loss')
            elif 'zand' in s_str or 'sand' in s_str: 
                soil_keys.append('zand_zuid' if ('zuid' in r_str or 'south' in r_str) else 'zand_nwc')
            else: 
    ...
    def calculate_corrected_tan(self):
        ...
        for method_name, uun_columns, mineralization_columns in [
            ...
            df[f'Total_Corrected_TAN_{method_name}'] = (
                df[f'Corrected_TAN_Cow_{method_name}'] + df[f'Corrected_TAN_Kalf_{method_name}'] + df[f'Corrected_TAN_Pink_{method_name}']
            )
...
class EmissionCalculator:
    ...
    def calculate_emissions(self):
        ...
        animal_categories_configuration = [
            {
                'animal_name': 'Cow', 'grazing_days_col': 'total_grazing_days_cow', 'grazing_hours_col': 'GH_Koe', 'slurry_fraction_col': 'slurry%_koe',
            ...
            {
                'animal_name': 'Pink', 'grazing_days_col': 'GD_Unlimited_Pink', 'grazing_hours_col': 'GH_Pink', 'slurry_fraction_col': 'slurry%_pink',
            ...
            {
                'animal_name': 'Kalf', 'grazing_days_col': 'GD_Unlimited_Kalf', 'grazing_hours_col': 'GH_Kalf', 'slurry_fraction_col': 'slurry%_kalf',
...
class LandApplicationCalculator:
    ...
    def _get_soil_key(soil_type, region):
        ...
        if 'zand' in soil_str: return 'zand_zuid' if ('zuid' in region_str or 'south' in region_str) else 'zand_nwc'
        return 'zand_nwc'
...
def run_pipeline(INPUT_PATH='InputREMAS.xlsx', OUTPUT_PATH='Output_REMAS_Complete.xlsx', SHEET='Main input'):
    ...
    # Run interconnected modules sequentially
    df = NitrogenPartitioningCalculator(df).calculate_partitioning()
    df = VEMRequirementCalculator(df).calculate_requirements()
    
    # Grassland biological model triggers right before feed allocation
    df = GrasslandProductionCalculator(df).calculate_production()
    ...
    # Write the fully processed DataFrame to Excel
