...
class FarmManureCalculator:
    ...
    def calculate_totals(self):
        ...
        # --- PRE-CALCULATE GRAZING & INDOOR FRACTIONS FOR ALL MODULES ---
        # Cow
        ...
        # KLW grazing reduction factor (avoid division by zero)
        ...
        # Pink (Heifers)
        df['grazing_days_frac_pink'] = (df['GD_Unlimited_Pink'] / 365.0).clip(0, 1)
        ...
        # Kalf (Calves)
        df['grazing_days_frac_kalf'] = (df['GD_Unlimited_Kalf'] / 365.0).clip(0, 1)
        ...
        df['vol_kalf_solid'] = df['Nr_kalf'] * fc * df['volume_solid_kalf'] * df['indoor_frac_kalf']
        df['vol_pink_slurry'] = df['Nr_pink'] * sp * df['volume_slurry_pink'] * df['indoor_frac_pink']
        df['vol_pink_solid'] = df['Nr_pink'] * fp * df['volume_solid_pink'] * df['indoor_frac_pink']
        ...
        df['vol_grazing_kalf_solid'] = df['Nr_kalf'] * fc * df['volume_solid_kalf'] * df['grazing_time_frac_kalf']
        df['vol_grazing_pink_slurry'] = df['Nr_pink'] * sp * df['volume_slurry_pink'] * df['grazing_time_frac_pink']
        df['vol_grazing_pink_solid'] = df['Nr_pink'] * fp * df['volume_solid_pink'] * df['grazing_time_frac_pink']
        
        df['Total_Grazing_Volume_m3'] = (df['vol_grazing_koe_slurry'] + df['vol_grazing_koe_solid'] +
                                         df['vol_grazing_kalf_slurry'] + df['vol_grazing_kalf_solid'] + 
        ...
        df['net_n_kalf_slurry'] = df['Nr_kalf'] * sc * df['slurry_kalf'] * df['indoor_frac_kalf']
        df['net_n_kalf_solid'] = df['Nr_kalf'] * fc * df['solid_kalf'] * df['indoor_frac_kalf']
        df['net_n_calves'] = df['net_n_kalf_slurry'] + df['net_n_kalf_solid']
        
        df['net_n_pink_slurry'] = df['Nr_pink'] * sp * df['slurry_pink'] * df['indoor_frac_pink']
        df['net_n_pink_solid'] = df['Nr_pink'] * fp * df['solid_pink'] * df['indoor_frac_pink']
        ...
        df['net_n_grazing_kalf_slurry'] = df['Nr_kalf'] * sc * df['slurry_kalf'] * df['grazing_time_frac_kalf']
        df['net_n_grazing_kalf_solid'] = df['Nr_kalf'] * fc * df['solid_kalf'] * df['grazing_time_frac_kalf']
        df['net_n_grazing_calves'] = df['net_n_grazing_kalf_slurry'] + df['net_n_grazing_kalf_solid']
        
        df['net_n_grazing_pink_slurry'] = df['Nr_pink'] * sp * df['slurry_pink'] * df['grazing_time_frac_pink']
        df['net_n_grazing_pink_solid'] = df['Nr_pink'] * fp * df['solid_pink'] * df['grazing_time_frac_pink']
        df['net_n_grazing_heifers'] = df['net_n_grazing_pink_slurry'] + df['net_n_grazing_pink_solid']
...
class VEMAllocationCalculator:
    ...
    def _soil_p(st):
        ...
        if 'klei' in s:   
            return {'yield_gs':10005, 'yield_fg':8893, 'yield_ms':17685, 'yield_nat_gs':6000, 'yield_nat_fg':5333, 'vem_gs_cult':960.0, 'vem_gs_nat':842.0, 'vem_fg_cult':960.0, 'vem_fg_nat':860.0, 'vem_maize':950.0}
        elif 'zand' in s: 
            return {'yield_gs':9360, 'yield_fg':8320, 'yield_ms':17773, 'yield_nat_gs':6000, 'yield_nat_fg':5333, 'vem_gs_cult':960.0, 'vem_gs_nat':842.0, 'vem_fg_cult':960.0, 'vem_fg_nat':860.0, 'vem_maize':950.0}
        elif 'veen' in s: 
            return {'yield_gs':9751, 'yield_fg':8668, 'yield_ms':16620, 'yield_nat_gs':6000, 'yield_nat_fg':5333, 'vem_gs_cult':957.0, 'vem_gs_nat':842.0, 'vem_fg_cult':960.0, 'vem_fg_nat':860.0, 'vem_maize':950.0} 
        else:
            return {'yield_gs':9700, 'yield_fg':9700, 'yield_ms':17300, 'yield_nat_gs':6000, 'yield_nat_fg':6000, 'vem_gs_cult':960.0, 'vem_gs_nat':842.0, 'vem_fg_cult':960.0, 'vem_fg_nat':860.0, 'vem_maize':950.0}
    ...
    def run_allocation(self):
        ...
        harvest_gs_vem = (self.df['DS_GS_Harvest_Actual'] * self.df['vem_gs_harvest']) / 1000
        harvest_ms_vem = (self.df['DS_MS_Harvest_Actual'] * self.df['vem_ms_harvest']) / 1000
        ...
        self.df['Total_Intake_Farm_kVEM'] = self.df['Total_Intake_Cow_kVEM'] + self.df['Total_Intake_Kalf_kVEM'] + self.df['Total_Intake_Pink_kVEM']
        self.df['VEM_Balance_Check'] = self.df['Total_Intake_Farm_kVEM'] - self.df['Total_VEM_Requirement_Farm_kVEM']

        print(f"  ✓ 2.2 done — Farm1 Balance Error (Should be ~0): {self.df['VEM_Balance_Check'].iloc[0]:.2f} kVEM")
        return self.df
    ...
    def _get_soil_nitrogen_content(self, soil_type):
        ...
        if 'klei' in soil_str:   
            return {
                ...
                'n_content_fresh_grass': 34.08, 
                'n_content_maize_silage': 12.0, 
                'n_content_natural_grass_silage': 24.7, 
                'n_content_natural_fresh_grass': 30.24
            } 
    ...
    def run_nutrient_calculation(self):
        ...
        for animal_category in ('cow', 'pink', 'kalf'):
            ...
            feed_matrix = [
                ...
                (f'DS_ms_{animal_category}', 'N_cont_ms_soil', 'maize_silage', False, 'MaizeSilage'),
                (f'DS_conc_{animal_category}', 'N_Concentrate_Avg', 'concentrate', False, 'Concentrate')
            ]
                     
            if animal_category == 'kalf':
                feed_matrix.append(('DS_milk_kalf', n_content_milk_dry_matter, 'dairy', True, 'Milk'))
                feed_matrix.append(('DS_kunst_kalf', self.N_CONTENT_MILK_REPLACER, 'dairy', True, 'KunstMelk'))
                
            if animal_category == 'cow':
                feed_matrix.append(('DS_Total_Byproducts_Cow', 'N_Byproducts_Avg', 'concentrate', False, 'Byproducts')) 
                feed_matrix.append(('DS_Total_OtherSilage_Cow', 'N_OtherSilage_Avg', 'grass_silage', False, 'OtherSilage'))
                
            total_n = pd.Series(0.0, index=self.df.index)
            total_cp = total_n.copy()
            total_vre = total_n.copy()
            
            for dry_matter_col, nitrogen_src, feed_type, is_dairy, export_label in feed_matrix:
                ...
                n_intake_kg = dry_matter * nitrogen_content / 1000.0
